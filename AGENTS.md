...
## Animations
All animations use `react-native-reanimated` (v4). Never use the React Native built-in `Animated` API.

### Entry Animation Tokens
| Block | Animation | Duration | Delay |
|-------|-----------|----------|-------|
| Full page / logo | `FadeIn` | 300–500ms | 0 |
| First content section | `FadeInDown` | 400ms | ~80–100ms |
| Each subsequent block | `FadeInDown` | 400ms | +80ms per block |
| Conditional/dynamic content | `FadeInDown` | 400ms | 0 (no delay) |

### Usage
```tsx
import Animated, { FadeIn, FadeInDown } from 'react-native-reanimated';

// Wrap major content sections — never SafeAreaView or ScrollView itself
...
### Rules
- Wrap major content sections only — not every single View
- Never animate `SafeAreaView` or `ScrollView` directly
- Stagger delays by ~80ms per block for a cascade effect
- Dynamic/conditional content (e.g. AI output) gets no delay — just duration
- List items cap stagger at 5 × 80ms to avoid long waits
